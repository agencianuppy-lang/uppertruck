	...
	if ($_POST){
		...
		*/
		if ($_FILES['att-foto']['name'] == ''){
			...
			$sql = $pdo->prepare("UPDATE blog SET title = '$title', titulo = '$titulo', artigo = '$descricao', categoria='$categoria', slug ='$slug', description='$description', keywords='$keywords' WHERE id = '$idNot'");
			$sql->execute();
			echo "ok";
		}else{
			if (!empty($_POST['Ncategoria'])){
