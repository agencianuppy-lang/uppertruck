	...
	if ($_POST){
		...
		*/
		if ($_FILES['imagem']['name'] == ''){
			...
			$sql = $pdo->prepare("UPDATE departamentos SET titulo = '$titulo', descricao = '$descricao' WHERE id = '$idDepto'");
			$sql->execute();
			echo "ok";
		}else{
			...
			$sqlS = $pdo->prepare("SELECT img FROM departamentos WHERE id = '$idDepto'");
			$sqlS->execute();
			...
			$cam = dirname(dirname(__FILE__)).'/imagens/'.$idDepto.'/';
			...
			// Caminho do BD
			$caminhoBD = 'imagens/'.$idDepto.'/'.$nomeFoto;

			// Atualiza a Noticia
			$sql = $pdo->prepare("UPDATE departamentos SET titulo = '$titulo', img = '$caminhoBD', descricao = '$descricao' WHERE id = '$idDepto'");
			$sql->execute();
			echo "ok";
		}
	}else{
