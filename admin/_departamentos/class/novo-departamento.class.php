<?php
	...
	if ($_POST){
		...
		$sql = $pdo->prepare("INSERT INTO departamentos (titulo, descricao, hora_postagem) VALUES ('$titulo', '$descricao', '$hora_postagem')");
		$sql->execute();
		$id = $pdo->lastInsertId();
		...
		$sqlU = $pdo->prepare("UPDATE departamentos SET img = '$caminhoBD' WHERE id = '$id'");
		$sqlU->execute();
