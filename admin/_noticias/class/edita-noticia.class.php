	...
	if ($_POST){
		...
		*/
		if ($_FILES['att-foto']['name'] == ''){
			...
			$sql = $pdo->prepare("UPDATE noticias SET titulo = '$titulo', noticia = '$descricao' WHERE id = '$idNot'");
			$sql->execute();
			echo "ok";
		}else{
			...
			$sqlS = $pdo->prepare("SELECT img FROM noticias WHERE id = '$idNot'");
			$sqlS->execute();
			...
			// Atualiza a Noticia
			$sql = $pdo->prepare("UPDATE noticias SET titulo = '$titulo', img = '$caminhoBD', noticia = '$descricao' WHERE id = '$idNot'");
			$sql->execute();
			echo "ok";
		}
	}else{
