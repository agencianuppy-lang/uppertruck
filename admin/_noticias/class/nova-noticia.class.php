<?php
	...
	if ($_POST){
		...
		$sql = $pdo->prepare("INSERT INTO noticias (titulo, noticia, hora_postagem) VALUES ('$titulo', '$descricao', '$hora_postagem')");
		$sql->execute();
		$id = $pdo->lastInsertId();
		...
		$sqlU = $pdo->prepare("UPDATE noticias SET img = '$caminhoBD' WHERE id = '$id'");
		$sqlU->execute();
