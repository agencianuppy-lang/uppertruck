<?php
	...
	if ($_POST){
		...
		$descricao = $_POST['test-descricao'];
		...
		$sql = $pdo->prepare("INSERT INTO testemunhos (titulo, nome, imagem, descricao, data) VALUES ('$titulo', '$nome', '', '$descricao', '$hora_postagem')");
		$sql->execute();
		$id = $pdo->lastInsertId();
		...
		$sqlU = $pdo->prepare("UPDATE testemunhos SET imagem = '$caminhoBD' WHERE id = '$id'");
		$sqlU->execute();
