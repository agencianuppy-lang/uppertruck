...
$position = (($page_number-1) * $item_per_page);
  ...
  $total = 1;
  $noticias = $class->Select(  "id, titulo, img, noticia, hora_postagem", "noticias", "ORDER BY id DESC", "LIMIT $position, $item_per_page"  );  
  while  ($row = $noticias->fetch(PDO::FETCH_OBJ)){  
    ...
    /*Imagem da noticia*/
    $imgN = "https://$server/admin/_noticias/".$row->img;
...
?> 
 ...
 <div id="item_<?= $row->id ?>" class="col-lg-<?= $col ?> card">
    <div class="card">
       <a href="https://<?= $server ?>/noticia/<?= $row->id ?>">
