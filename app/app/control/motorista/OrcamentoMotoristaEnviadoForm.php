<?php
class OrcamentoMotoristaEnviadoForm extends TPage
{
    protected $form;      // form
    ...
    public function __construct()
    {
        ...
        $this->form_orcamento->setFormTitle('Dados do seu orçamento');
        ...
        $valor_orcamento->setEditable(FALSE);
        $prazo_orcamento->setEditable(FALSE);
        $obs_orcamento->setEditable(FALSE);
        ...
        //$this->form_orcamento->addHeaderAction( 'Cancelar orçamento',     new TAction([$this, 'onCancelOrcamento']),   'fa:times red');
        ...
        $vbox->add(new TXMLBreadCrumb('menu.xml', 'OrcamentoMotoristaEnviadoList'));
        $vbox->add($this->form);
    ...
    function onEdit($param)
    {
        try
        {
            if (isset($param['id']))
            {
                ...
                $programacao = $object->programacao;
                if ($orcamento[0]->status != 3 ) //não for do status aceito
                {
                   $this->form_orcamento->addHeaderAction( 'Cancelar orçamento',     new TAction([$this, 'onCancelOrcamento']),   'fa:times red');
                   $this->form->addHeaderAction( 'Rejeitar solicitação', new TAction([$this, 'onReject']), 'far:trash-alt red');
                    //new TMessage('info', json_encode($programacao->id));
                }
                if ($programacao AND $orcamento[0]->status == 3 )
                {
                
                     $filter = ['id' => $programacao->id]; 
                     $this->form->addHeaderAction( 'Programação de embarque',  new TAction(['ProgramacaoMotoristaList', 'onSearch'], ['filter_search' => $filter] ),  'fa:hand-point-right #7C93CF');
                }
    ...
    public static function onCancelOrcamento($param)
    {
        // define the delete action
        $action = new TAction(array(__CLASS__, 'CancelOrcamento'));
        $action->setParameters($param); // pass the key parameter ahead
        // shows a dialog to the user
        new TQuestion('Você realmente quer cancelar seu envio de orçamento?', $action);
    }
    
   
    function CancelOrcamento($param)
    {
        try
        {
            ...
            $object = new OrcamentoMotorista($key);  // create an empty object
            $object->data_orcamento           = NULL;
            //$object->cotacao_id             = $cotacao->id;
            //$object->motorista_id           = TSession::getValue('userid');
            $object->status                   = 1;  // Pendente de análise por parte da uppertruck
            $object->valor                    = NULL;
            $object->prazo                    = NULL;
            $object->obs                      = NULL;
            ...
            $act = new TAction(['OrcamentoMotoristaEnviadoList','onReload']);
            new TMessage('info', 'Envio de orçamento cancelado com sucesso.', $act);
