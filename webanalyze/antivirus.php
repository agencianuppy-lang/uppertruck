...
class SGAntiVirus_scanner
{
	...
	public function scanner($check_session = true, $show_results = true)
	{
		...
		if (file_exists($this->tmp_dir.'report_unwanted_files.php')) unlink($this->tmp_dir.'report_unwanted_files.php');
                ...
                if (is_dir($file)) 
                {
					...
					if (strpos($file, '.php') || strpos($file, '.phtml'))
					{
                        ...
                        switch ($ext)
                        {
                            ...
                            case 'so':
                            case 'exe':
                                self::DebugLog('Unwanted file: '.str_replace($this->scan_path, "", $file));
...
';
	...
	public function UnwantedFilesLog($txt)
	{
        $file = $this->tmp_dir.'report_unwanted_files.php';
        if (!file_exists($file)) $txt = "<?php die(); ?>\n".$txt;
	    $fp = fopen($file, 'a');
		fwrite($fp, $txt."\n");
		fclose($fp);
